...
//		System.out.println(intArray[3]); // 없는 인덱스라 에러
		
		// trt catch 쓰는이유 오류가 나도 프로그램이 계속 실행됨
		try {
		...
		// 예외처리 코드 블럭
		// 우클릭 - Surround With - try catch 클릭
		try {
...
//		}
		...
		try {
			ExMethod.printName("유정000"); // 이 메소드는 에러를 던질 수 도 있는 메소드입니다.
		} catch (BizException e) { // throw new BizException 객체가 들어온다.
			e.printStackTrace();
			System.out.println(e.getMessage());
			System.out.println(e.getErrCode());
			// 에러페이지로 전환
		} catch (Exception e) { // 미처 발견하지 못한 에러를 위한 보험
			e.printStackTrace();
		}
		...
		try {
			...
			System.out.println("항상 실행되는 명령어");
		}
